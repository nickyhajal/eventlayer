<script lang="ts">
  ...
  let eventFor = eventForOptions.find(
    (t) => t.value === (event.eventFor || ""),
  );
...
<form on:submit={createEvent}>
  <div
    class="grid {inDialog
      ? 'grid-cols-1 px-0'
      : 'grid-cols-[24rem_24rem] px-4'} gap-8"
  >
    <div>
      {#if inDialog}
        <Dialog.Header>
      ...
      <div class="grid gap-4 py-4">
        ...
        {#if !simplified}
          ...
          {#if event?.eventFor?.includes("rsvp")}
            <div class="flex flex-col items-start justify-center gap-1">
              <Label for="event_maxAttendees" class="text-right"
                >Max Attendees</Label
              >
              <Input
          ...
          {#if event?.eventFor?.includes("rsvp") || event.eventFor.includes("selected")}
            <div
              class="mt-0 flex items-center justify-between rounded-md border bg-stone-50 p-3"
            >
              <label
                for="showAttendeeList"
                class="text-sm font-medium text-stone-800"
                >Show Attendee List on Event Page</label
              >
              <Switch
                name="showAttendeeList"
                bind:checked={event.showAttendeeList}
                class=""
              />
            </div>
          {/if}
      ...
      <div class="grip mt-9 gap-4 px-4 py-4">
        ...
        >
          {#each users || [] as user}
            ...
            <div
              class="group relative flex items-center justify-between gap-2 px-2.5 py-2"
            >
          ...
          {:else}
            <div class="text-sm text-center text-gray-500 py-6">
              No users yet.
            </div>
          {/each}
        </div>
        <div class="rounded-lg bg-stone-100 p-2">
          <div class="mb-0.5 pl-0.5 text-sm font-medium text-gray-500">
            Add People
          </div>
        ...
        <div class="mt-4">
          <Label for="event_internalNotes" class="text-right"
