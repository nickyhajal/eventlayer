<script lang="ts">
  ...
  import { capitalize, dayjs, truncate } from "@matterloop/util";
  ...
  const columns: ColumnDef<Event>[] = [
    ...
    },
    {
      accessorKey: "name",
      header: "Event",
      cell: (info) => truncate(info.getValue() as string, { length: 60 }),
    },
    {
      accessorKey: "host",
      header: "Host",
      accessorFn: (row) => {
        const user = row?.users?.find((u) => u.type === "host")?.user;
    ...
    },
    {
      ...
      cell: (info) =>
        info.getValue()
          ? dayjs(info.getValue()).format("MMM. Do [at] h:mma")
          : "-",
      header: () => "Starts",
    },
  ];
  ...
  const csvFields = [
    ...
    { key: "numAttendees", label: "# Attending" },
    {
      ...
      label: "Starts At",
      accessor: (row) =>
        row.startsAt ? dayjs(row.startsAt).format("YYYY-MM-DD HH:mm") : "",
    },
    {
      key: "endsAt",
      label: "Ends At",
      accessor: (row) =>
        row.endsAt ? dayjs(row.endsAt).format("YYYY-MM-DD HH:mm") : "",
      default: false,
    },
