<script lang="ts">
  ...
  type EventInfoField = {
    key: string;
    label: string;
    fieldType: "text" | "textarea" | "boolean" | "options";
    options?: string;
  };
  ...
  export let eventInfoFields: EventInfoField[] = [];
  let error = "";
  ...
  function inferFieldType(value: string): "text" | "textarea" | "boolean" {
  ...
  $: hiddenEventInfoKeys = new Set([
    ...knownInfoKeys,
    ...customFields.map((field) => field.key),
    "firstName",
    "lastName",
  ]);
  $: inferredEventInfoFields = Object.entries(user.info || {})
    .filter(([key]) => !hiddenEventInfoKeys.has(key))
    .map(([key, value]) => ({
      key,
      label: humanizeKey(key),
      fieldType: inferFieldType(value?.value ?? ""),
    }));
  $: eventInfoFieldMap = new Map<string, EventInfoField>();
  $: {
    for (const field of inferredEventInfoFields) {
      eventInfoFieldMap.set(field.key, field);
    }
    for (const field of eventInfoFields) {
      if (!field.key || hiddenEventInfoKeys.has(field.key)) continue;
      eventInfoFieldMap.set(field.key, field);
    }
  }
  $: mergedEventInfoFields = Array.from(eventInfoFieldMap.values()).sort(
    (a, b) => a.label.localeCompare(b.label),
  );
  $: buttonMsg = emailConfirmed
    ? user?.id
      ? "Save User"
      : "Add User"
    : "Check Email";
  ...
  function resolveType(value?: string) {
    ...
    return (
      userTypes.find((t) => t.value === value) || {
        value,
        label: toLabel(value),
      }
    );
  ...
  $: {
    ...
    if (
      user &&
      (userKey !== lastUserKey ||
        customFieldsKey !== lastCustomFieldsKey ||
        eventInfoFieldsKey !== lastEventInfoFieldsKey)
    ) {
      lastUserKey = userKey;
      lastCustomFieldsKey = customFieldsKey;
      lastEventInfoFieldsKey = eventInfoFieldsKey;
      error = "";
      userInEvent = false;
...
{#if user?.status !== "active" && !simplified}
  <div
    class="mb-4 rounded-md bg-red-50 p-3 text-center text-sm font-semibold text-red-700/80"
  >
...
<form on:submit|preventDefault={saveUser}>
  ...
  <div class="grid gap-4 py-4">
    ...
    {#if emailConfirmed !== "existing-user"}
      <div
        class="flex flex-col items-start justify-center gap-1 {emailConfirmed ===
        'create-user'
      ...
      {#if !simplified}
        <div class="flex gap-2">
          <div class="flex flex-col items-start justify-center gap-1">
            ...
            <Input
              id="company"
              bind:value={user.info.company.value}
              class="col-span-3"
            />
          </div>
          <div class="flex flex-col items-start justify-center gap-1">
            ...
            <Input
              id="title"
              bind:value={user.info.title.value}
              class="col-span-3"
            />
          </div>
        </div>
        <div class="flex flex-col items-start justify-center gap-1">
          <Label for="url" class="text-right"
            >User Url (LinkedIn, Website, etc)</Label
          >
          <Input
            id="url"
            type="text"
        ...
        <div class="flex flex-col items-start justify-center gap-1">
          ...
          <Textarea
            id="bio"
            bind:value={user.info.bio.value}
            class="col-span-3"
          />
        </div>
        <!-- <div class="flex flex-col items-start justify-center gap-1">
        ...
        {#if customFields.length > 0}
          <div class="mt-4 border-t border-stone-200 pt-4">
            <Label class="mb-3 block text-sm font-semibold text-stone-700"
              >Custom Fields</Label
            >
            {#each customFields as field (field.id)}
              <div class="mb-3 flex flex-col items-start justify-center gap-1">
                <Label for={`custom-${field.key}`} class="text-right"
                  >{field.label}</Label
                >
                {#if field.fieldType === "text"}
                  <Input
                    ...
                    on:input={(e) =>
                      setCustomFieldValue(field.key, e.currentTarget.value)}
                    class="col-span-3"
                  />
                ...
                {:else if field.fieldType === "options"}
                  ...
                  <Select.Root
                    selected={options.includes(getCustomFieldValue(field.key))
                      ...
                      : undefined}
                    onSelectedChange={(v) =>
                      setCustomFieldValue(field.key, v?.value || "")}
                  >
        ...
        {/if}
        {#if mergedEventInfoFields.length > 0}
          <div class="mt-4 border-t border-stone-200 pt-4">
            <Label class="mb-3 block text-sm font-semibold text-stone-700"
              >User Submitted Info</Label
            >
            {#each mergedEventInfoFields as field (field.key)}
              <div class="mb-3 flex flex-col items-start justify-center gap-1">
                <Label for={`event-info-${field.key}`} class=""
                  >{field.label}</Label
                >
                {#if field.fieldType === "textarea"}
                  <Textarea
                    id={`event-info-${field.key}`}
                    value={getCustomFieldValue(field.key)}
                    on:input={(e) =>
                      setCustomFieldValue(field.key, e.currentTarget.value)}
                    class="col-span-3"
