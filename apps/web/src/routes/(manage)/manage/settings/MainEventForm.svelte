  ...
  >
    <div>
      {#if showTitle}
        ...
        {:else}
          <div class={tw(`mb-2 mt-0 text-2xl font-semibold ${titleClass}`)}>
            {title} Settings
          </div>
        {/if}
      {/if}
      <div class="grid gap-4 py-4">
        ...
        {/if}
        <div class="flex flex-col items-start justify-center gap-1">
          <Label for="event_replyEmail" class="text-right"
            >Reply Email Address</Label
          >
          <Input
            id="event_replyEmail"
            bind:value={event.replyEmail}
            class="col-span-3"
          />
        </div>
        <div class="flex flex-col items-start justify-center gap-1">
          ...
          <Input
            id="event_fromName"
            bind:value={event.emailFromName}
            class="col-span-3"
          />
        </div>
        <div class="flex flex-col items-start justify-center gap-1">
        ...
        <div class="flex flex-col items-start justify-center gap-1">
          <Label for="event_settings_groove_api_key" class="text-right"
            >Groove API Key</Label
          >
          <Input
    ...
    {#if !simplified && event?.id}
      <div class="mt-9 grid gap-4 px-4 py-4">
        ...
        <div>
          <div class="flex flex-col items-start justify-center gap-1 pb-1">
            <Label for="image" class="text-right">Dark Mode Logo</Label>
          </div>
          <div
            class="flex flex-col items-center gap-2 overflow-hidden rounded-lg border border-stone-200 bg-stone-50 p-1"
          >
            {#if event.darkLogoId}
              <img src={getMediaUrl(event.darkLogo)} alt="at" class="w-full" />
            {/if}
            <div class="w-full bg-stone-100">
              <Uploader
                parentId={event.id}
                parentType="event-dark-logo"
                onSuccess={(mediaId) => updateMedia(mediaId, "darkLogoId")}
              />
            </div>
          </div>
        </div>
