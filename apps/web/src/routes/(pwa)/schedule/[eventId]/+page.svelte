<script lang="ts">
  ...
  $: users = orderBy(
    shouldGroup
      ? data.users
      : data.users.map((row) => ({ ...row, type: "attendee" })),
    ["type"],
    "desc",
  );
  $: rsvpd = $me?.rsvps?.find(
    (rsvp) =>
      rsvp.event.startsAt.replace("T", " ") ===
  ...
  $: canRsvp =
    event?.maxAttendees === 0 || users.length < (event?.maxAttendees || 0);
  let lastType = "";
...
<Screen
  ...
  bigTitle={event.photo || event?.venue?.photo ? " " : ""}
  back="/schedule"
  photo={event.photo || event?.venue?.photo}
>
  <div class="shell mx-auto max-w-7xl pb-20 pt-safe-offset-3">
    ...
    {#if event.subtitle}
      <div
        class="-mt-2 pb-3 pr-6 text-base font-semibold leading-snug text-slate-600"
      >
        {event.subtitle}
      </div>
    {/if}
    ...
    {#if users?.length > 0 && event?.showAttendeeList}
      <div class="mt-3 flex flex-col gap-2">
        {#if !shouldGroup}
          <div
            class="mb-0 mt-2 text-lg font-semibold text-a-accent brightness-95"
          >
        ...
        {#each users as user}
          {#if shouldGroup}
            {#if getLastType(user)}
              ...
              >
                {plural(
                  users.filter((u) => u.type === user.type).length,
