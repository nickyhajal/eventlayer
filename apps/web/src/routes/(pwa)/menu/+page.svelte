<script lang="ts">
  ...
  // Lock page scroll only via overflow + overscroll — never touch-action:none (that can block taps).
  onMount(() => {
...
<Screen title="Menu" photo={$event?.photo}>
  <div class="mx-auto min-h-[calc(100vh-5rem)] max-w-7xl">
    <div class="flex flex-col justify-end gap-2 pt-2">
      ...
      <div
        class="menu fixed grid w-[calc(100dvw-1.5rem)] touch-manipulation grid-cols-2 gap-2 lg:relative lg:w-full"
      >
