<script lang="ts">
  ...
  onMount(() => {
    if (typeof window !== "undefined") {
      ...
      window.scrollTo({
        top:
          document.getElementById(`event-${foundNextEventAt}`)?.offsetTop + 87,
        behavior: "smooth",
      });
    }
  });
  ...
  $: visibleEvents =
    showPrevious || nextEvents.length === 0
...
<Screen title="Meetups" bigTitle="Meetups">
  <div class="mx-auto max-w-7xl px-2">
    <div class="">
      ...
      <div
        class="relative w-[calc(100dvw-2rem)] pb-[5vh] lg:w-[calc(100%-2rem)]"
      >
        <div
        ...
        {#if previousEvents.length > 0}
          <div class="flex w-full justify-center">
            <button
              ...
              class="mx-auto mb-0 mt-6 w-1/2 rounded-full bg-slate-100/80 px-2 py-1 text-sm font-medium text-slate-500"
        ...
        {#each visibleEvents as event, i}
          ...
          {#if isNewDay}
            <div class="my-4 pt-2 flex items-center gap-3 px-1">
          ...
          {#if isNew}
            ...
            >
              ...
              <div>
                {dayjs(event.startsAt).format("h:mma").replace("m", "")}
              </div>
            </div>
          {/if}
          <div class="relative grid grid-cols-[4.4rem_1fr]" id="event-{i}">
            <div class="relative">
              {#if !isNew}
                <div
                  class="relative -top-4 left-7 h-full w-1 bg-slate-100"
                ></div>
              {/if}
            </div>
            <EventRow {event} />
