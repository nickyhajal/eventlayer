  ...
  function filterUsers(users: User[], showType: string) {
    if (showType === "my-friends") {
      return users.filter((user) =>
        data.me.connectionsTo.some((c) => c.toId === user.id),
      );
    }
    ...
    <div class="mt-4 grid grid-cols-1 gap-1.5 py-2 lg:grid-cols-2 lg:gap-4">
      {#each sortUsers(filterUsers(users, showType)) as user}
        ...
        >
          <a
            href="/user/{id}"
            class="mb-2 mt-3 flex items-center gap-4 px-2 lg:flex-col"
          >
          ...
          >
            <a
              href="/user/{id}"
              class="flex h-full w-1/2 items-center justify-center text-center"
              >View Profile</a
            >
          </div>
        </div>
