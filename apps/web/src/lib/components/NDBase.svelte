<img
	src="https://coyklkkrxa.cloudimg.io/_e/prod/event-large-logo/a92f8ee4-8257-4e4b-a52a-e15681828436-1.png?v=1&"
	class="w-32 mx-auto mb-16 mt-16"
/>
<div class="fixed w-full top-0 left-0 h-full">
	<div
		class="ball-blue w-72 absolute -right-[220px] md:-right-[120px] bottom-[12%] md:bottom-[20%]"
	></div>
	<div class="ball-oj w-48 absolute -left-[150px] md:-left-[60px] top-[8%] md:top-[12%]"></div>
</div>
<div class="bg-image fixed w-full top-0 left-0 z-10 h-full opacity-30"></div>

<style>
	.bg-image {
		background-image: url('https://framerusercontent.com/images/EErxJcrmRQPI1Li7WAZRMIU.png');
		background-size: cover;
		background-position: center;
		background-repeat: no-repeat;
	}
	.ball-blue {
		aspect-ratio: 1 / 1;
		background: linear-gradient(215.32372915759623deg, #22b2d7 0%, rgba(46, 208, 195, 1) 100%);
		border-radius: 100%;
		flex: none;
		z-index: 1;
	}
	.ball-oj {
		aspect-ratio: 1 / 1;
		background: linear-gradient(215.32372915759623deg, #dd4b4d 0%, rgba(247, 184, 77, 1) 100%);
		border-radius: 100%;
		z-index: 1;
	}
</style>
