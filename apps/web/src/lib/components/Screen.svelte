<script lang="ts">
  ...
  function handleContentScroll(e) {
    const scrollY =
      e.currentTarget === contentElm ? contentElm.scrollTop : window.scrollY;
    if (bigTitle) {
      if (scrollY > 0) {
...
>
  ...
  {#if !noBgScreen}
    <div
      class="fixed top-0 z-10 block h-64 w-full bg-slate-800 lg:hidden"
    ></div>
  {/if}
    ...
    <div class=" lg:ml-16 lg:max-w-3xl">
      ...
      {#if bigTitle}
        ...
        >
          {#if photo}
            <div
              class="absolute top-0 h-24 w-full bg-gradient-to-b from-black/50 to-black/0"
            ></div>
            <div
            ...
            {#if titleSelectOptions}
              <Popover.Root bind:open={titleSelectMobileOpen}>
                ...
                >
                  ...
                  >
                    {titleSelectOptions?.find(
                      ({ value }) => value === titleSelectValue,
                    )?.label || ""}
                  </div>
                  <ChevronDown
                    class="h-8 w-8 px-0 text-white hover:bg-transparent"
                  ></ChevronDown>
                </Popover.Trigger>
            ...
            {#if titleSelectOptions}
              <Popover.Root bind:open={titleSelectOpen}>
                ...
                >
                  ...
                  <ChevronDown
                    class="h-8 w-8 px-0 text-slate-500/70 hover:bg-transparent"
                  ></ChevronDown>
                </Popover.Trigger>
